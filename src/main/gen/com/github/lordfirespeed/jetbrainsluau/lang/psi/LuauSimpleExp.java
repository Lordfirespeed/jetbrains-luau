// This is a generated file. Not intended for manual editing.
package com.github.lordfirespeed.jetbrainsluau.lang.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface LuauSimpleExp extends PsiElement {

  @Nullable
  LuauAnonymousFunction getAnonymousFunction();

  @Nullable
  LuauIfElseExp getIfElseExp();

  @Nullable
  LuauNumberLiteral getNumberLiteral();

  @Nullable
  LuauPrefixExp getPrefixExp();

  @Nullable
  LuauStringInterp getStringInterp();

  @Nullable
  LuauStringLiteral getStringLiteral();

  @Nullable
  LuauTableConstructor getTableConstructor();

}
